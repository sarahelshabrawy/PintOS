...
list_wakeup_time_comp(const struct list_elem*a,const struct list_elem*b, void*aux UNUSED){
  ...
  const int64_t b_member = list_entry(b, struct thread, elem)->wakeupTime;
...
timer_sleep (int64_t ticks) 
{
  ...
  list_insert_ordered(&sleeping_threads, &t->elem, list_wakeup_time_comp, NULL);
  thread_block();
  intr_set_level (old_level);
}
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  while(!list_empty(&sleeping_threads)) {
    ...
    if(timer_ticks() < t->wakeupTime)
      break;
    list_remove(e);
    thread_unblock(t);
    //reschedule if any threads woke up
    reschedule = true;
  }
  if(reschedule)
